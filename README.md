...
**所以其实我们可以得出一个结论：我列的这台机器，主板其实配错了（笑）**。想要硬盘发挥最大性能，其实不能使用这里的 Z790-A；作为正确答案之一，可以将主板换成 Z790-E，因为该卡提供了一个 PCIE 5.0x4 级别的M.2接口。
...
如之前提到的，在DX12中，内存分配的处理变得更加精细。比如要分配一张纹理，在DX11时，我们可以简单的调用CreateTexture2D实现，由驱动帮我们自动完成：分配共享显存-分配显存-从共享显存拷贝到显存 的流程；但在DX12时，这一步骤完全需要开发者自己处理，这就把很多人整不会了。

其实对一个简单的，无顶点动画的DX Demo而言，涉及资源分配的内容，基本可以抽象成以下几种：

- （无动画的）顶点、索引数据
- 材质的参数
- WVP等各种矩阵、时间、设备类型等全局参数
- 纹理
- RT
- 描述符
...
（无动画的）顶点、索引数据：可以在首个使用这套 VB/IB 的 Mesh初始化的时候，新建顶点缓存；


其实，大可以把问题想得简单一点，我们首先编写一个按页分配的内存池。

要分配描述符的时候，要构建描述符堆，根据运行时需求往里装填描述符；当我们要分配纹理的时候，可以构建资源池；
